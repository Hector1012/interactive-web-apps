...
const style = document.querySelector('[data-settings-theme]').value = 'dark' ? night : day
...
document.querySelector('[data-search-cancel]').addEventListener('click', () => {
  ...
  document.querySelector('[data-list-button]').addEventListener('click', () => {
    ...
    document.querySelector('[data-list-button]').disabled = !(matches.length - (page + 1) *
    BOOKS_PER_PAGE > 0);
  page++;
});
...
document.querySelector('[data-settings-form]').addEventListener('submit', (event) => {
  ...
  document.documentElement.style.setProperty('--color-dark', style[result.theme.dark]);
  document.documentElement.style.setProperty('--color-light', style[result.theme.light]);
  document.querySelector('[data-settings-overlay]').close() = false;
  
});

document.querySelector('[data-list-items]').addEventListener('click', (event) => {
    const trackArray = Array.from(event.path || event.composedPath());
    let active = null;
  
    for (const node of trackArray) {
